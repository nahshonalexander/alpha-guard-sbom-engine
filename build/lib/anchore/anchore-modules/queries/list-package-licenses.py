...
try:
    config = anchore_utils.init_query_cmdline(sys.argv, "params: <license string> <license string> ...\nhelp: use 'all' to show all package/licenses")
...
try:
    ...
    adata = anchore_utils.load_analysis_output(config['imgid'], 'package_list', 'pkgs.allinfo')
    for pname in adata.keys():
        ...
        for k in pkgdata.keys():
            #TODO: what are base strings?
            if isinstance(pkgdata[k], str):
