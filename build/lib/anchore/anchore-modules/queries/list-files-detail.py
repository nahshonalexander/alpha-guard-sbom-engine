...
try:
    config = anchore_utils.init_query_cmdline(sys.argv, "params: <directory prefix> <directory prefix> ...\nhelp: use 'all' to show all files")
...
try:
    ...
    detail_data = anchore_utils.load_analysis_output(config['imgid'], 'file_list', 'files.allinfo')
    checksum_data = anchore_utils.load_analysis_output(config['imgid'], 'file_checksums', 'files.sha256sums')
    if not checksum_data:
        checksum_data = anchore_utils.load_analysis_output(config['imgid'], 'file_checksums', 'files.md5sums')
