...
try:
    ...
    idata = anchore_utils.load_image_report(config['imgid'])
    for fid in idata['familytree']:
        fdata = anchore_utils.load_image_report(fid)
        ftreetypes[fid] = fdata['meta']['usertype']

    ddata = anchore_utils.load_analysis_output(config['imgid'], 'layer_info', 'layers_to_dockerfile')
    ...
    for record in ddata_list:
        ...
        if not inherited:
            if layer != config['imgid']:
                (fromline, fromid) = anchore_utils.discover_from_info(idata['dockerfile_contents'])
