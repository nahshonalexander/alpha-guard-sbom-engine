...
class AnchoreConfiguration (object):
    ...
    #TODO: Change from StandardError To Runtime for now. look at why that was a bad idea
    def restore(self, dest_root, backup_file):
        if not os.path.exists(dest_root):
            raise RuntimeError('Destination root dir does not exist')

        if isinstance(backup_file, str) and not os.path.exists(backup_file):
            raise RuntimeError('Backup file %s not found' % backup_file)
