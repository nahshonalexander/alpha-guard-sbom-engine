...
norm_packages = anchore_utils.normalize_packages(imageId)
...
for pkg in vermatch:
    if pkg in bin_packages:
        if bin_packages[pkg]['fullvers'] != vermatch[pkg]:
            rc = anchore_utils.compare_package_versions(imageId, pkg, bin_packages[pkg]['fullvers'], pkg, vermatch[pkg])
            if rc < 0:
