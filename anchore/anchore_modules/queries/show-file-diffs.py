    ...
    for fid in fids:
        try:
            ...
            if fid == 'base':
                ...
                try:
                    fimageId = anchore_utils.discover_imageId(fid)
                except ValueError as err:
                    warns.append("Cannot lookup imageId specified as input parameter: " + fid)

            if fimageId:
                if not anchore_utils.is_image_analyzed(fimageId):
                    raise Exception("imageId ("+str(fimageId)+") is not analyzed or analysis failed")

                fimage = anchore_image.AnchoreImage(fimageId, allimages=allimages)
                image_report = anchore_utils.diff_images(image.meta['imageId'], fimage.meta['imageId'])
...
if len(warns) > 0:
    anchore_utils.write_plainfile_fromlist(config['output_warns'], warns)
