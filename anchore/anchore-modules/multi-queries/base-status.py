...
try:
    config = anchore_utils.init_query_cmdline(sys.argv, "params: <base image ID> <base image ID> ...\nhelp: use 'all' to show all base image IDs")
except:
    sys.exit(1)
...
for name in config['params']:
    if name != 'all':
        try:
            imageId = anchore_utils.discover_imageId(name)
        except:
            imageId = name
    else:
        imageId = 'all'
...
for imageId in config['images']:
    try:
        idata = anchore_utils.load_image_report(imageId)
