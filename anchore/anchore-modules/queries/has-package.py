...
try:
    config = anchore_utils.init_query_cmdline(sys.argv, "params: <package> <package> ...\nhelp: search input image(s) for specified <package> installations")
...
try:
    pkgs = anchore_utils.load_analysis_output(config['meta']['imageId'], 'package_list', 'pkgs.all')
