...
try:
    ...
    image_data = anchore_utils.load_image(imageId)

    result = anchore_utils.load_analysis_output(imageId, 'package_list', 'pkgs.all')
    numpkgs = len(result.keys())

    result = anchore_utils.load_analysis_output(imageId, 'file_list', 'files.all')
    numfiles = len(result.keys())

    result = anchore_utils.load_analysis_output(imageId, 'file_list', 'files.nonpkged')
